...
> operator *after* resting. All four would display as FIRM under a naive check. Under our policy they
> mint UNVERIFIED depth instead, which makes no claim.
>
...
sixth (late operator grant) is rested on-chain with its exact blocked state documented. See [DEMO.md](DEMO.md) and
...
**Built and passing — `forge test`: 93 tests, 0 failures · `src/` at 100% line coverage (168/168)**

Plus **5 symbolic proofs** (`npm run prove`) that quantify over every caller and every
timestamp at once, rather than sampling them.
...
- `src/FirmnessRegistry.sol` — the ternary classifier (**FIRM / PULLABLE / UNVERIFIED**) expressed as
  ...
  horizon. (22 tests.) **Deployed and seeded on Shannon 2026-08-26** at
  [`0x04aDbfC40dD10215Ee7b7D14B0aD74074a83f8C1`](https://shannon-explorer.somnia.network/address/0x04aDbfC40dD10215Ee7b7D14B0aD74074a83f8C1)
  — so `classify(address)` is a call any other Somnia contract can make, not just a script we run.
  See [Verify the registry yourself](#verify-the-registry-yourself).
...
- `script/` — the off-chain engine: a dependency-free `keccak256`, an EVM disassembler + static
  ...
  the **firmness %** over a live market, and the **bench** (retype p95 **0.13 ms** on a deterministic
  2,000-order book, inside a 100 ms block). `node script/test.mjs` → 17 checks, including four that
  pin the analyzer's known evasions; the analyzer's hash matches on-chain `EXTCODEHASH`.
- `src/IBinaryPool.sol` — the pool surface, transcribed from `@somnia-chain/markets-sdk`, and
  **checked against it in CI** by `script/sdk-verify.mjs` (35/35). See below.
...
> owner was the contract.* The permanent proof is the mined transaction below — transaction history
...
npm run verify                   # syntax + lint + typecheck + 17 off-chain checks + headline 8/8
...
# 3. Contracts: build + test  → 90 passing, 0 failures
forge build
forge test
...
# 5. One command that runs everything: syntax, lint, typecheck, 17 checks, headline 8/8
npm run verify
```
...
## Zero runtime dependencies — *and* no silent drift from the sponsor SDK
